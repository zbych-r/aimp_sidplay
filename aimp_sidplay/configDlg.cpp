...
void ConfigDlgInitDialog(HWND hWnd)
{
	...
	CThreadSidPlayer* sidPlayer = CSidplayPlugin::Instance()->CreatePlayerInstance();
...
void UpdateConfig(HWND hWnd)
{
	...
	if (strlen(buf) == 0)
	{
		playerConfig->playlistFormat = new char[1];
		playerConfig->playlistFormat[0] = 0;
	}
	else
	{
	...
	if (strlen(buf) == 0)
	{
		playerConfig->subsongFormat = new char[1];
		playerConfig->subsongFormat[0] = 0;
	}
	else
	{
	...
	case WM_COMMAND:
		{
			...
			case IDOK:
				UpdateConfig(hWnd);
				sidPlayer = CSidplayPlugin::Instance()->CreatePlayerInstance();
